...
// 1. THIS IS THE ENGINE THAT CONNECTS TO YOUR SANITY DATABASE
...
// 2. THIS TOOL TURNS SANITY IMAGE DATA INTO REAL IMAGE URLS
const builder = imageUrlBuilder(client);
function urlFor(source: any) {
  return builder.image(source);
}

// 3. NOTICE WE ADDED "async" HERE SO THE PAGE CAN WAIT FOR THE DATABASE
export default async function Shop() {
  // 4. FETCH THE LIVE PRODUCTS FROM SANITY!
  const products = await client.fetch(`*[_type == "product"]`);

  // 5. YOUR WHATSAPP NUMBER (Without the '+' symbol)
          ...
          <div className="flex flex-col lg:flex-row gap-8 mt-4">
            ...
            <div className="flex-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {/* 6. LOOP THROUGH THE LIVE PRODUCTS HERE */}
                {products.map((item: any) => (
                  <div
                    key={item._id}
                    className="bg-white/80 backdrop-blur-xl rounded-[2rem] overflow-hidden border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(16,185,129,0.1)] hover:-translate-y-1.5 transition-all duration-500 group flex flex-col"
                  >
                    {/* Live Image Area */}
                    ...
                    <div className="p-6 flex flex-col flex-1 bg-white relative z-10">
                      ...
                      <div className="mt-auto flex items-center justify-between">
                        ...
                        <a
                          href={`https://wa.me/${myWhatsAppNumber}?text=Hi Glow Global Mart, I am interested in buying: ${item.name} (₹${item.price}). Is it available?`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="bg-emerald-50 border border-emerald-100 text-emerald-700 hover:bg-emerald-600 hover:text-white p-3 rounded-2xl transition-all duration-300 shadow-sm"
